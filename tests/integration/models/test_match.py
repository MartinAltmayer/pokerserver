...
from pokerserver.models import (Player, get_all_cards, Match, Table, NotYourTurnError, PositionOccupiedError,
                                InsufficientBalanceError, InvalidBetError)
...
class TestCheck(BettingTestCase):
    ...
    async def test_check_changes_current_player(self):
        await self.async_setup(bets=[1, 2, 2, 2])
        await self.match.check(self.players[3].name)
        table = await Table.load_by_name(self.table.name)
        self.assertEqual(self.players[0], table.current_player)
...
class TestRaise(BettingTestCase):
    ...
    @gen_test
    async def test_raise_zero(self):
        await self.async_setup()
        with self.assertRaises(InvalidBetError):
            await self.match.raise_bet(self.players[3].name, 0)
