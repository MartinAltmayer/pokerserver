...
class TestUUIDController(IntegrationHttpTestCase):
    ...
    async def test_uuid(self, _, add_uuid_mock):
        response = await self.post('/uuid', body={'player_name': 'hans'})
    ...
    async def test_uuid_invalid_name(self):
        response = await self.post('/uuid', body={'player_name': 'aa'}, raise_error=False)
        self.assertEqual(response.code, HTTPStatus.BAD_REQUEST.value)

    @patch('pokerserver.database.UUIDsRelation.add_uuid', side_effect=return_done_future(exception=DuplicateKeyError()))
    @patch('uuid.uuid4', return_value='123-456')
    @gen_test
    async def test_uuid_fail_on_duplicate(self, *_):
        response = await self.post('/uuid', body={'player_name': 'hans'}, raise_error=False)
        self.assertEqual(response.code, HTTPStatus.BAD_REQUEST.value)
